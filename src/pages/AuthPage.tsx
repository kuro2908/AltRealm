...
function getErrorMessage(code: string): string {
  switch (code) {
    ...
    case "auth/weak-password": return "Mật khẩu quá yếu (ít nhất 6 ký tự).";
    case "auth/invalid-email": return "Email không hợp lệ.";
    case "auth/popup-closed-by-user": return "";
    case "auth/too-many-requests": return "Quá nhiều lần thử. Vui lòng đợi một lúc.";
    ...
    <div className="min-h-screen bg-background flex items-center justify-center px-4">
      ...
      >
        ...
        <div className="flex items-center justify-center gap-2 mb-8">
          ...
          <span className="text-lg font-semibold text-foreground tracking-tight">StoryWeaver</span>
        </div>
        ...
        <div className="rounded-lg bg-card p-6 shadow-elevated">
          ...
          <p className="text-sm text-muted-foreground text-center mb-6">
            {mode === "login" ? "Đăng nhập để tiếp tục viết" : "Bắt đầu hành trình của bạn"}
          </p>
          ...
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-2 rounded-lg border border-border bg-background px-4 py-2.5 text-sm font-medium text-foreground hover:bg-secondary transition-sw mb-4 disabled:opacity-50"
          >
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <button
              type="submit"
              disabled={loading}
              className="block w-full text-center rounded-lg bg-primary text-primary-foreground px-4 py-2.5 text-sm font-medium hover:opacity-90 transition-sw disabled:opacity-50"
            >
